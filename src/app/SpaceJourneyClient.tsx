...
function SceneCoasting({ progress, stars, data }: { progress: MotionValue<number>; stars: Star[]; data: PhaseContent["hyperspace"] }) {
  // Visible: 0.12 -> 0.26
  const opacity = useTransform(progress, [0.12, 0.13, 0.25, 0.26], [0, 1, 1, 0]);
  const pointerEvents = useTransform(progress, (p) => p > 0.13 && p < 0.25 ? "auto" : ("none" as any));
  
  // Text visible from 0.14 to 0.24
  const textY = useTransform(progress, [0.14, 0.16, 0.22, 0.24], [40, 0, 0, -40]);
  const textOpacity = useTransform(progress, [0.14, 0.15, 0.23, 0.24], [0, 1, 1, 0]);

  const endTextOpacity = useTransform(progress, [0.23, 0.24, 0.25, 0.26], [0, 1, 1, 0]);
  ...
  return (
    <motion.div style={{ position: "absolute", inset: 0, zIndex: 20, background: "#000", opacity, pointerEvents, display: "flex", justifyContent: "center", alignItems: "center", overflow: "hidden" }}>
      
      {/* Slow coasting starfield */}
      <motion.div style={{ position: "absolute", inset: "-5%", y: starsY, width: "110%", height: "110%" }}>
      ...
      >
        <div className="info-box" style={{ background: "rgba(8,12,20,0.85)", borderColor: "rgba(100,200,255,0.4)" }}>
      ...
      >
        <div style={{ textAlign: "center", padding: "2rem", background: "rgba(0,0,0,0.5)", backdropFilter: "blur(10px)", borderRadius: "20px", border: "1px solid rgba(100,200,255,0.1)" }}>
          ...
          <p style={{ fontFamily: "var(--font-display)", fontSize: "0.8rem", letterSpacing: "0.6em", color: "var(--accent)", marginTop: "0.5rem" }}>
            COASTING TO MOON
          </p>
        </div>
      </motion.div>
...
function SceneMoonApproach({ progress, stars }: { progress: MotionValue<number>; stars: Star[] }) {
  // Visible: 0.68 -> 0.84
  const opacity = useTransform(progress, [0.68, 0.69, 0.83, 0.84], [0, 1, 1, 0]);
...
function CustomScrollItem({ progress, item }: { progress: MotionValue<number>; item: CustomPhaseItem }) {
  const FADE_DUR = 0.02;
  ...
  const pointerEvents = useTransform(progress, (p) => p >= item.startScroll && p <= item.endScroll ? "auto" : ("none" as any));
  ...
  return (
    ...
    >
      <div className="info-box" style={{ background: "rgba(8,12,20,0.85)", borderColor: "var(--accent2)" }}>
  ...
  return (
    <div style={{ width: "100%", height: "20000vh", background: "#000", fontFamily: "var(--font-inter)", position: "relative" }}>
      ...
      <div style={{ position: "fixed", inset: 0, zIndex: 10, overflow: "hidden" }}>
        ...
        <SceneCoasting progress={smoothProgress} stars={stars} data={phasesData.hyperspace} />

        {/* 0.24 - 0.40 : Debris Orbit (Meteor Phase) */}
        <DebrisPhase progress={smoothProgress} stars={stars} data={phasesData.debris} />
